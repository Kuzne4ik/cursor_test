...
if ($element_in_first_frame->is_exist()) {
    ...
    echo "Element inner text: " . $element_in_first_frame->get_inner_text() . "\n";
} else {
    echo "\n\nElement not found in the first frame\n";
}

// Example 3: Search in the second frame by number
$element_in_second_frame = DOM::$anchor->get_by_attribute('id', 'frame2_anchor', 1);

if ($element_in_second_frame->is_exist()) {
    echo "\n\nFound element in the second frame (frame = 1)\n";
    echo "Element inner text: " . $element_in_second_frame->get_inner_text() . "\n";
} else {
    echo "\n\nElement not found in the second frame\n";
}

// Example 4: Search in a nested frame
...
if ($element_in_nested_frame->is_exist()) {
    echo "\n\nFound element in a nested frame (frame = \"0:1\")\n";
    echo "Element inner text: " . $element_in_nested_frame->get_inner_text() . "\n";
} else {
    echo "\n\nElement not found in the nested frame\n";
}

// Example 5: Search in a deeply nested frame
...
if ($element_in_deeply_nested_frame->is_exist()) {
    echo "\n\nFound element in a deeply nested frame (frame = \"1:0:2\")\n";
    echo "Element inner text: " . $element_in_deeply_nested_frame->get_inner_text() . "\n";
} else {
    echo "\n\nElement not found in the deeply nested frame\n";
}

// Example 6: Using frame argument with other element types
$button_in_frame = DOM::$button->get_by_name('submit', 0);

if ($button_in_frame->is_exist()) {
    echo "\n\nFound button in the first frame (frame = 0)\n";
    echo "Button name: " . $button_in_frame->get_name() . "\n";
} else {
    echo "\n\nButton not found in the first frame\n";
}
