...
$success = DOM::$listbox->select_text_by_attribute_by_form_name("name", "contries", true, "Canada", true, "form1", -1);
...
// Select an option by text in a select element within a form, found by attribute and form name with frame parameter
// Parameters:
// - $attr_name: The name of attribute to search by (e.g., "id", "class")
// - $attr_value: The value of attribute to match
// - $exactly: Whether to match the attribute value exactly (true) or partially (false)
...
$successWithFrame = DOM::$listbox->select_text_by_attribute_by_form_name("name", "contries", true, "USA", true, "form1", 0);

if ($successWithFrame) {
    echo "Successfully selected the option with text 'USA' in the select element with name 'contries' within form 'form1' in frame 0\n\n";
} else {
