<?php
// Scenario: Select an option by text content in a select element found by its attribute
// Description: Demonstrates how to select an option by its text content from a select element based on its attribute
...
$success = DOM::$listbox->select_text_by_attribute("name", "contries", true, "Canada", true, -1);
...
// Select an option by its text content in a select element found by its attribute with frame parameter
// Parameters:
// - $attr_name: The name of the attribute to search by (e.g., "id", "class")
// - $attr_value: The value of the attribute to match
// - $exactly: Whether to match the attribute value exactly (true) or partially (false)
...
$successWithFrame = DOM::$listbox->select_text_by_attribute("name", "contries", true, "USA", true, 0);
