<?php
// Scenario: Move the cursor to a specific position in a textarea element found by attribute
...
WEB::$browser->wait_js(5, "1");

// Find the textarea by attribute
$attr_name = "id";
$attr_value = "w3review";
$exactly = true;
$position = 20; // Position to move cursor to
...
if (DOM::$textarea->is_exist_by_attribute($attr_name, $attr_value, $exactly))
{
    ...
    $result = DOM::$textarea->seek_to_pos_by_attribute($attr_name, $attr_value, $exactly, $position);
    
    // Display the result
    if ($result)
    {
        echo "\nSuccessfully moved cursor to position $position in textarea by attribute";
    }
    else
    {
        echo "\nFailed to move cursor to position $position in textarea by attribute";
    }
}
else
{
