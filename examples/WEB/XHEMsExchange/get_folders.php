<?php
// Scenario: Get folders from MS Exchange
...
try{
    ...
    if ($isConnected) {
        // Example 1: Get Inbox folders
        echo("\nExample 1: Get Inbox folders: ");
        $inboxFolderName = "Входящие";
        print_r(WEB::$msexchange->get_folders($inboxFolderName));

        // Example 2: Get root folders (empty name)
        echo("\nExample 2: Get root folders (empty name): ");
        $rootFolderName = "";
        print_r(WEB::$msexchange->get_folders($rootFolderName));

        // Example 3: Get root folders (system name)
        echo("\nExample 3: Get root folders (system name): ");
        $systemRootFolderName = "msgfolderroot";
        print_r(WEB::$msexchange->get_folders($systemRootFolderName));
    }
}
finally
{
