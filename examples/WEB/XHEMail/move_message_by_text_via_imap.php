<?php
// Scenario: Demonstrates moving messages by text via IMAP
...
try {
    ...
    if ($isConnected) {
        ...
        if ($messagesCount == 0) {
            ...
            echo(WEB::$mail->add_message_via_imap($targetFolder, $imapLogin, $imapLogin, $subjectSearch, $textSearch, 0));
        }
        ...
        // Example 1: Move message by text
        echo("\nExample 1: Move message with text '$textSearch' from folder '$targetFolder': ");
        echo(WEB::$mail->move_message_by_text_via_imap($targetFolder, "test2", $textSearch, false));

        // Step 6: Get final message count
        echo("\nStep 6: Get message count in folder '$targetFolder': ");
        $messagesCount = WEB::$mail->get_message_count_via_imap($targetFolder);
        echo($messagesCount);
