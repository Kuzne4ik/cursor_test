<?php
// Scenario: Demonstrates getting messages by ID via IMAP with multiple email providers
...
function call(string $imap_server, int $imap_port, string $imap_login, string $imap_password, int $ssl_option, string $cert_type, int $timeout, string $log_path, string $target_folder): void
{
    try {
        ...
        if ($isConnected) {
            ...
            echo("\nStep 5: Get message by number in folder '$target_folder': \n");
            $message = WEB::$mail->get_message_by_number_via_imap($target_folder, 0);
            ...
            echo("Example 1: Get message by ID = '$message->message_id' in folder '$target_folder': ");
            $message = WEB::$mail->get_message_by_id_via_imap($target_folder, $message->message_id);
