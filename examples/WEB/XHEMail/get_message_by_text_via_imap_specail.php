<?php
// Scenario: Demonstrates getting messages by text via IMAP with multiple email providers
 ...
 * @return void
 */
function call(string $imap_server, int $imap_port, string $imap_login, string $imap_password, int $ssl_option, string $cert_type, int $timeout, string $log_path, string $target_folder, string $text_search): void
{
    try {
        ...
        if ($isConnected) {
            ...
            // Example 1: Get message by text
            echo("\nExample 1: Get message by text '$text_search' in folder '$target_folder': ");
            $message = WEB::$mail->get_message_by_text_via_imap($target_folder, $text_search);
            if (!empty($message->from)) {
                print_r($message->text_body);
...
call($imapServer, $imapPort, $imapLogin, $imapPassword, $sslOption, $certType, $timeout, $logPath, $targetFolder, $textSearch);
