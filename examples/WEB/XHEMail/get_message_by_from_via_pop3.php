<?php
// Scenario: Get message by from field using POP3 protocol

$xhe_host = "127.0.0.1:7010";
if (!isset($path)){
    // Path to the init.php file for connecting to the XHE API
    $path = "../../../Templates/init.php";
    // Including init.php grants access to all classes and functionality for working with the XHE API
    require($path);
}

// Start
echo "\n<font color=blue>webpage->".basename (__FILE__)."</font>\n";

// Step 1 - Set proxy
$proxyUrl = "socks://127.0.0.1:9150";
WEB::$mail->set_proxy($proxyUrl);

// Step 2
echo "Step 2. Set POP3 configuration: ";
$pop3Server = "pop.mail.ru";
$pop3Port = 995;
$username = "arna.pervak";
$password = "msnijwdx";
echo WEB::$mail->pop3_connect($pop3Server, $pop3Port, $username, $password)."\n";

// Example 1
echo "Example 1. Get message containing @corp.mail.ru in from field: ";
$fromSearch = "@corp.mail.ru";
$message = WEB::$mail->get_message_by_from_via_pop3($fromSearch);
echo "\n\nfrom: ".$message->from."\n";
echo "\n\nsubject: ".$message->subject."\n";
echo "\n\ndate: ".$message->date."\n";
echo "\n\nbody length: ".strlen($message->body)."\n\n";

// Example 2
echo "Example 2. Get message 1 containing @corp.mail.ru in from field: ";
$exactMatch = false;
$messageIndex = 1;
$message = WEB::$mail->get_message_by_from_via_pop3($fromSearch, $exactMatch, $messageIndex);
echo "\n\nfrom: ".$message->from."\n";
echo "\n\nsubject: ".$message->subject."\n";
echo "\n\ndate: ".$message->date."\n";
echo "\n\nbody length: ".strlen($message->body)."\n\n";

// Step 3
echo "Step 3. Get number of messages: ";
echo WEB::$mail->get_message_count_via_pop3()."\n";

// Step 4
echo "Step 4. Delete message 0 containing @corp.mail.ru in from field: ";
//echo WEB::$mail->delete_message_by_from_via_pop3($fromSearch)."\n";

// Step 5
echo "Step 5. Get number of messages: ";
echo WEB::$mail->get_message_count_via_pop3()."\n";

// End
echo "\n";

// Quit the application
WINDOW::$app->quit();
?>