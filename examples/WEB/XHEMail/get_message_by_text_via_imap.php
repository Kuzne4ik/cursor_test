<?php
// Scenario: Demonstrates getting messages by text via IMAP
...
try {
    ...
    if ($isConnected) {
        ...
        echo("\nExample 1: Get message by text '$textSearch' in folder '$targetFolder': ");
        $message = WEB::$mail->get_message_by_text_via_imap($targetFolder, $textSearch);
        if (!empty($message->from)){print_r($message->text_body);
