<?php
// Scenario: Demonstrates getting messages by from field via IMAP with multiple email providers
 ...
 * @param string $from_search
 * @return void
 */
function call(string $imap_server, int $imap_port, string $imap_login, string $imap_password, int $ssl_option, string $cert_type, int $timeout, string $log_path, string $target_folder, string $from_search): void
{
    try {
        ...
        if ($isConnected) {
            ...
            // Example 1: Get message by from field
            echo("\nExample 1: Get message by from field in folder '$target_folder': ");
            $message = WEB::$mail->get_message_by_from_via_imap($target_folder, $from_search);
