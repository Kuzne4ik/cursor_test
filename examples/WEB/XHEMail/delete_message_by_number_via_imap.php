<?php
// Scenario: Delete message by number using IMAP protocol
...
try {
    ...
    if ($isConnected) {
        ...
        if ($messagesCount == 0) {
            ...
            $body = "test";
            $priority = 0;
            var_export(WEB::$mail->add_message_via_imap($targetFolder, $from, $to, $subject, $body, $priority));
                   
            // Step 4
            echo("\nStep 4. Get number of messages in folder '$targetFolder': ");
            echo(WEB::$mail->get_message_count_via_imap($targetFolder));
        }

        // Example 1
        echo("\n\nExample 1. Delete message 0 in folder '$targetFolder': ");
        $messageNumber = 0;
        echo(WEB::$mail->delete_message_by_number_via_imap($targetFolder, $messageNumber));
