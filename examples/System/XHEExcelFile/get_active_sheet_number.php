<?php
$xhe_host = "127.0.0.1:7010";

// Connect functional objects if not already connected
if (!isset($path)){
    // Path to the init.php file for connecting to the XHE API
    $path = "../../../../../Templates/init.php";
    // Including init.php grants access to all classes and functionality for working with the XHE API
    require($path);
}

// Scenario: Demonstrate setting active sheet and getting active sheet number in Excel file
echo "\n<span style=\"color: blue; \">excelfile->" . basename(__FILE__) . "</span>\n";

// Initialize Excel
SYSTEM::$excel->kill();

// Example 1: Set active sheet by number
echo("Example 1: Set active sheet by number : ");
// Extract method arguments into variables
$filePath = "test/test.xlsx";
$sheetIndex = 2;
$result = SYSTEM::$excelfile->set_active_sheet_by_number($filePath, $sheetIndex);
if ($result === true)
    echo("true\n");
else
    echo("false\n");

// Example 2: Get the number of the active sheet
echo("\nExample 2: Get the number of the active sheet : ");
$activeSheetNumber = SYSTEM::$excelfile->get_active_sheet_number($filePath);
echo($activeSheetNumber . "\n");

// View result


// End of script
echo "\n";

// Quit application
WINDOW::$app->quit();
?>
