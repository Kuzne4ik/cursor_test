...
if ($notepadWindows->count() > 0) {
    ...
    // step 2: Resize the window to make it larger
    $newWidth = $initialWidth + 100;
    $newHeight = $initialHeight + 50;
    echo "Resize Notepad window: ";
    ...
    if ($isResized) {
        ...
        // step 3: Move the window to a new position
        $newX = $initialX + 50;
        $newY = $initialY + 50;
        
        // Example move: Move the window to the new position
        echo "Example move: Move Notepad window: ";
        $isMoved = $targetNotepadWindow->move($newX, $newY);
        if ($isMoved) {
            echo "Notepad window moved successfully to X=$newX, Y=$newY\n";
            
            // Verify the new position
            $currentX = $targetNotepadWindow->get_x();
            $currentY = $targetNotepadWindow->get_y();
            echo "Current Notepad window position: X=$currentX, Y=$currentY\n";
            ...
            if ($isMaximized) {
                ...
                // step 5: Restore the window to its previous size
