<?php
// Scenario: Getting the top-level parent window of a child window (Left Pane in XHE).
...
if ($xheWindows->count() > 0) {
    ...
    if ($leftPane->is_exist()) {
        ...
        // Example get_top_parent: Get the top-level parent window of the Left Pane
        echo "Example get_top_parent: Get top-level parent window of Left Pane: ";
        $topParentWindow = $leftPane->get_top_parent();
        if ($topParentWindow->is_exist()) {
            $topParentText = $topParentWindow->get_text();
            echo "Top-level parent window text: '$topParentText'\n";
        } else {
            echo "Failed to get top-level parent window or top-level parent does not exist.\n";
        }
    } else {
