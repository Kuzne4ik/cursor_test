<?php
// Scenario: Getting the parent window of a child window (Left Pane in XHE).
...
if ($xheWindows->count() > 0) {
    ...
    if ($leftPane->is_exist()) {
        ...
        // Example get_parent: Get the parent window of the Left Pane
        echo "Example get_parent: Get parent window of Left Pane: ";
        $parentWindow = $leftPane->get_parent();
        if ($parentWindow->is_exist()) {
            $parentText = $parentWindow->get_text();
            echo "Parent window text: '$parentText'\n";
        } else {
            echo "Failed to get parent window or parent does not exist.\n";
        }
    } else {
