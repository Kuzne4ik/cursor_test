<?php
// Scenario: Demonstrates how to perform a right mouse click on a UI element
...
if ($quickStartElement && $quickStartElement->is_exist()) {
    // Example 1: Right click on the Quick Start element with offset
    $offsetX = 2;
    $offsetY = 2;
    $result = $quickStartElement->mouse_right_click($offsetX, $offsetY);
    
    if ($result) {
        echo("Example 1: Successfully right clicked on Quick Start element at offset ($offsetX, $offsetY)\n");
        echo("Context menu should now be visible\n");
        
        // Step: Wait a moment and then hide the context menu
        sleep(2);
        $escapeKeyCode = 27;
        SYSTEM::$keyboard->key($escapeKeyCode);
        echo("Context menu hidden with Escape key\n");
    } else {
        echo("Example 1: Failed to right click on Quick Start element\n");
    }
} else {
