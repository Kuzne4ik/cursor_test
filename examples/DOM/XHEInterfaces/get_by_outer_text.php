...
// Scenario: Get elements by outer text from a collection
// Description: This example demonstrates how to get a collection of elements and then find specific elements by their outer text within that collection
...
// Example: Get all anchor elements and then find specific anchors by their outer text within that collection
...
if ($anchors->count() > 0)
{
    ...
    // Try to find an anchor with a specific outer text (exact match) in anchor elements
    $targetText = "Example Link";
    $anchorByText = $anchors->get_by_outer_text($targetText, true);
    
    // Check if the anchor with the specified outer text was found in anchor elements and exists
    ...
    // Try to find an anchor with a partial outer text match in anchor elements
    $partialText = "Link";
    $anchorByPartialText = $anchors->get_by_outer_text($partialText, false);
    
    // Check if the anchor with the partial outer text was found in anchor elements and exists
    ...
    // Try to find an anchor with another outer text in anchor elements
    $targetText2 = "Test";
    $anchorByText2 = $anchors->get_by_outer_text($targetText2, true);
    ...
    // Try to find an anchor with a non-existent outer text in anchor elements
    $nonExistentText = "NonExistentLink";
    $anchorByNonExistentText = $anchors->get_by_outer_text($nonExistentText, true);
    
    // Check if the anchor with the non-existent outer text was found in anchor elements and exists
    ...
    // Display all anchor outer texts in the collection for reference
    ...
    for ($k = 0; $k < $anchors->count(); $k++)
    {
        ...
        {
            $anchorOuterText = $currentAnchor->get_outer_text();
            $anchorInnerText = $currentAnchor->get_inner_text();
            $anchorHref = $currentAnchor->get_href();
            
            echo "Anchor #" . ($k + 1) . ": Outer Text = '" . $anchorOuterText . "', Inner Text = '" . $anchorInnerText . "', Href = '" . $anchorHref . "'\n";
        }
    }
}
else
