...
// Scenario: Send key down events to elements in a collection using full emulation
// Description: This example demonstrates how to get a collection of input elements and send key down events to each element using full emulation
...
// Example: Get all input elements and send key down events to each element using full emulation
...
if ($inputs->count() > 0)
{
    ...
    // Process each input to send key down events to it
    for ($k = 0; $k < $inputs->count(); $k++)
    {
        ...
        if ($currentInput->is_exist())
        {
            ...
            if ($focusResult)
            {
                ...
                // Send a sequence of key down events for characters
                $textToSend = "T est";
                for ($j = 0; $j < strlen($textToSend); $j++)
                {
                    $charKey = substr($textToSend, $j, 1);
                    $result = $currentInput->key_down($charKey);
                    
                    // Check if the key down event was successful
                    if ($result)
                    {
                        ...
                        // Send key up event to complete the key press
                        $keyUpResult = $currentInput->key_up($charKey);
