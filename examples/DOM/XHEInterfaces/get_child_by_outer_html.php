...
// Scenario: Get a child element by its outer HTML from elements in a collection
// Description: This example demonstrates how to get a collection of elements and retrieve their child elements by outer HTML
...
// Example: Get all div elements and retrieve their child elements by outer HTML
...
if ($divs->count() > 0)
{
    ...
    // Try to find a child element with specific outer HTML (exact match)
    $searchHtml = "<span>"; // This should be adjusted based on the actual content
    $childByExactHtmls = $divs->get_child_by_outer_html($searchHtml, true);
    ...
    if (count($childByExactHtmls) > 0) {
        ...
        for ($k = 0; $k < $childByExactHtmlsCount; $k++) {
            ...
            if ($childByExactHtml->is_exist()) {
                ...
                echo "  Outer HTML: " . $childByExactHtml->get_outer_html() . "\n";
            } else {
                echo "No child found with exact outer HTML '" . $searchHtml . "'\n";
            }
        }
    }
    ...
    $childByPartialHtmls = $divs->get_child_by_outer_html($partialSearchHtml, false);
    ...
    if (count($childByPartialHtmls) > 0) {
        ...
        for ($k = 0; $k < $childByPartialHtmlsCount; $k++) {
            ...
            if ($childByPartialHtml->is_exist()) {
                ...
                echo "  Outer HTML: " . $childByPartialHtml->get_outer_html() . "\n";
            } else {
                echo "No child found containing outer HTML '" . $partialSearchHtml . "'\n";
            }
        }
    }
}
